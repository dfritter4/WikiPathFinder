...
public class WikiPhilosophyPagePathFindingServiceTests {
	...
	public void testKnownPathFromRockMusic() {
		when(cache.contains(anyString())).thenReturn(false);
		when(repo.findByStartingPage(anyString())).thenReturn(null);
		PathResponse response = service.getPathToPhilosophy("https://en.wikipedia.org/wiki/Rock_music");
		
		assertEquals(new Integer(9), response.getHopsOnPath());
		assertEquals(PATH_FROM_ROCK_MUSIC, response.getPathToPhilosophy());
	...
	public void testKnownPathFromCaptainAmerica() {
		when(cache.contains(anyString())).thenReturn(false);
		when(repo.findByStartingPage(anyString())).thenReturn(null);
		PathResponse response = service.getPathToPhilosophy("https://en.wikipedia.org/wiki/Captain_America");
		
		assertEquals(new Integer(6), response.getHopsOnPath());
		assertEquals(PATH_FROM_CAPTAIN_AMERICA, response.getPathToPhilosophy());
	...
	public void testPathDoesNotExistFromPaper() {
		when(cache.contains(anyString())).thenReturn(false);
		when(repo.findByStartingPage(anyString())).thenReturn(null);
		PathResponse response = service.getPathToPhilosophy("https://en.wikipedia.org/wiki/Paper");
		
		assertEquals(new Integer(-1), response.getHopsOnPath());
		assertEquals("", response.getPathToPhilosophy());
	...
	public void testPathDoesNotExistFromProton() {
		when(cache.contains(anyString())).thenReturn(false);
		when(repo.findByStartingPage(anyString())).thenReturn(null);
		PathResponse response = service.getPathToPhilosophy("https://en.wikipedia.org/wiki/Proton");
